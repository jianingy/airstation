...
use errors::*;
use models::{AirQuality, Environment};
...
pub fn init_table(db: &mut Connection) -> Result<()> {
    {
        ...
        try!(stmt.execute(()).chain_err(|| db_error!("cannot create table air_quality")));
    }
    {
        let mut stmt = try!(
            db.prepare("CREATE TABLE IF NOT EXISTS environment ( \
                        id SERIAL PRIMARY KEY, \
                        humidity INT, \
                        temperature INT, \
                        created_at TIMESTAMP, \
        ...
        try!(stmt.execute(()).chain_err(|| db_error!("cannot create table environment")));
    }
    Ok(())
}
...
pub fn add_environment(db: &mut Connection, humi: i32, temp: i32) -> Result<()> {
    let mut stmt = try!(
        db.prepare("INSERT INTO environment (humidity, temperature) \
                    VALUES(?,?)")
            .chain_err(|| db_error!("cannot insert data"))
    );
    try!(stmt.execute((&humi, &temp))
        .chain_err(|| db_error!("cannot insert data")));
    Ok(())
}
...
pub fn get_air_quality(db: &mut Connection, start: NaiveDateTime, end: NaiveDateTime, interval: u32)
    ...
    let records = try!(stmt.execute((interval, interval, start, end, interval))
...
pub fn get_environment(db: &mut Connection, start: NaiveDateTime, end: NaiveDateTime,
                       interval: u32) -> Result<Vec<Environment>> {
    let mut stmt = try!(
        db.prepare("SELECT
                    FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(created_at) / ?) * ?) AS created_at,
    ...
    let records = try!(stmt.execute((interval, interval, start, end, interval))
                       .chain_err(|| db_error!("SQL statement error")));
    let mut vec: Vec<Environment> = Vec::new();
    for record in records {
        if let Ok(mut row) = record {
            vec.push(Environment {
                humidity: row.take("humidity").unwrap(),
                temperature: row.take("temperature").unwrap(),
                created_at: row.take("created_at").unwrap()
            });
        }
    };
